...
const floatingCards = [
  { icon: BrainCircuit, title: "Intent classified", value: "Employment dispute", position: "left-6 top-6" },
  { icon: ShieldAlert, title: "Risk score", value: "Medium", position: "right-6 top-6" },
  { icon: Scale, title: "Recommended action", value: "Request written notice", position: "left-6 bottom-6" },
  { icon: BadgeCheck, title: "Reviewer queue", value: "Ready in 3 hours", position: "right-6 bottom-6" }
];
    ...
    <section className="relative overflow-hidden">
      <div className="mx-auto grid w-full max-w-7xl gap-16 px-4 py-16 sm:px-6 lg:grid-cols-[1.15fr,0.85fr] lg:px-8 lg:py-24">
        ...
        <div className="relative min-h-[520px]">
          ...
          <div className="absolute inset-x-12 top-20 bottom-24 rounded-[28px] border border-border/60 bg-background/90 p-6 shadow-soft">
          ...
          {floatingCards.map((item, index) => {
            ...
            return (
              <div
                className={`absolute ${item.position} z-10 hidden w-48 rounded-[24px] border border-white/50 bg-white/90 p-4 shadow-soft backdrop-blur-xl md:block dark:border-white/10 dark:bg-slate-950/70 xl:w-52`}
                key={item.title}
              >
