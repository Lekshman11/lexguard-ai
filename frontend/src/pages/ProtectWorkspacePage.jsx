...
function normaliseVoiceTranscript(transcript) {
...
function isStartProtectionCommand(transcript) {
  const normalized = normaliseVoiceTranscript(transcript);
  const exactPhrases = ["protect me", "start protection", "arm protection", "enable protection"];

  if (exactPhrases.some((phrase) => normalized.includes(phrase))) {
    return true;
  }

  return includesAllWords(normalized, ["protect"]) && (normalized.includes("start") || normalized.includes("arm"));
}

function isStopProtectionCommand(transcript) {
  const normalized = normaliseVoiceTranscript(transcript);
  const exactPhrases = [
    "stop protection",
    "stop protect mode",
  ...
  if (normalized.includes("stop production")) {
    return true;
  }

  return (
    (normalized.includes("stop") || normalized.includes("end") || normalized.includes("disable") || normalized.includes("cancel")) &&
    (normalized.includes("protect") || normalized.includes("protection") || normalized.includes("protect mode") || normalized.includes("production"))
  );
}
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (locationError) => {
        if (locationError?.code === 1) {
          setGeoStatus("Location permission was denied. Allow location access in the app prompt to enable SOS tracking.");
        } else if (locationError?.code === 2) {
          setGeoStatus("Device location is turned off. Approve the Android location prompt to enable GPS inside the app.");
        } else if (locationError?.code === 3) {
          setGeoStatus("Location request timed out. Keep GPS enabled and try once more.");
        } else {
  ...
  const toggleVoiceCommands = () => {
    ...
    if (!recognitionRef.current) {
      ...
      recognition.onresult = (event) => {
        ...
        if (isStartProtectionCommand(transcript)) {
          startProtection();
          return;
        }

        if (isStopProtectionCommand(transcript)) {
          stopProtection();
          return;
        }
